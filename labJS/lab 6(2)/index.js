...
const basketBtn = buttonService.createButton(
  basket,
  new Button({
    className: "basket",
    text: "Корзина",
...
const authBtn = buttonService.createButton(
  auth,
  new Button({
    className: "auth",
    text: "Авторизация",
...
function parallax(event) {
  this.querySelectorAll(".layer").forEach((element) => {
    element.style.transform = `translate(${event.clientX / 50}px, ${
      event.clientY / 50
    }px)`;
  });
}
